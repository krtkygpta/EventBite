...
def create_auth_view(page, is_register=False):
    ...
    username_field = ft.TextField(label="Username", **field_style)
    password_field = ft.TextField(label="Password", password=True, can_reveal_password=True, **field_style, on_submit=handle_auth)
    ...
    if is_register:
        ...
        controls.append(
            ft.Row([
                ft.FilledTonalButton(button_text, on_click=handle_auth, icon=ft.Icons.LOGIN),
                ft.OutlinedButton("Register", on_click=lambda _: page.go("/register"), icon=ft.Icons.PERSON_ADD_ALT_1)
            ], alignment=ft.MainAxisAlignment.CENTER)
        )

    if is_register:
        controls.append(ft.FilledTonalButton(button_text, on_click=handle_auth))
    ...
    return ft.View(
        route,
        [
            ft.AppBar(title=ft.Text("EventBite", style=ft.TextThemeStyle.HEADLINE_MEDIUM), center_title=True, bgcolor="surface_variant"),

            ft.Text(page_title, style=ft.TextThemeStyle.HEADLINE_MEDIUM),
            ft.Column(
                controls,
                alignment=ft.MainAxisAlignment.CENTER,
            ...
            content=ft.Stack([
                ...
                ft.Container(
                    ...
                    gradient=ft.LinearGradient(
                        begin=ft.alignment.bottom_center, end=ft.alignment.center,
                        colors=[ft.Colors.with_opacity(1, ft.Colors.BLACK), ft.Colors.with_opacity(0, ft.Colors.BLACK)]
                    ),
                    border_radius=10,
        ...
        [
            ...
            ft.Column(
                [
                    ...
                    ft.FilledTonalButton("Book Seats", on_click=lambda _: page.go(f"/seating/{event_id}"), icon=ft.Icons.BOOKMARK)
                ],
                spacing=10,
                horizontal_alignment=ft.CrossAxisAlignment.CENTER,
                width=page.width 
...
def create_homepage_view(page):
    ...
    # Create a class to hold the search state
    class SearchState:
        def __init__(self):
            self.query = ""
    ...
    def update_events_grid(tab_content, event_type):
        tab_content.content = create_events_grid(
            page, 
            event_type if event_type != 'All' else None,
            search_state.query if search_state.query else None
        )
        page.update()
    
    def on_search(e):
        search_state.query = e.control.value or ""
        for tab in tabs_control.tabs:
            update_events_grid(tab.content, tab.text)
    
    # Create search bar
    search_bar = create_search_bar(page, on_search)
    
    # Create a tab for each event type with its corresponding content
    tabs = []
    for event_type in event_types:
        tab_content = ft.Container(
            content=create_events_grid(page, event_type if event_type != 'All' else None),
            padding=16,
            expand=True
        )
        tabs.append(
    ...
    return ft.View(
        ...
        [
            ft.AppBar(
                title=ft.Text("EventBite", style=ft.TextThemeStyle.HEADLINE_LARGE),
                center_title=False,
                ...
                actions=[
                    ...
                    ft.IconButton(
                        ...
                        on_click=lambda e: (
                            page.client_storage.set("is_logged_in", False),
...
def create_events_grid(page, event_type, search_query=None):
    ...
    if search_query and search_query.strip():
        ...
        events = [
            e for e in events 
            if (e['EventName'] and search_lower in (e['EventName'] or '').lower()) or 
               (e.get('description') and search_lower in (e.get('description') or '').lower()) or
               (e.get('type') and search_lower in (e.get('type') or '').lower())
        ]
    ...
    if not events:
        return ft.Container(
            content=ft.Column([
                ...
                ft.Text("Try adjusting your search or filter", style=ft.TextThemeStyle.BODY_MEDIUM, color=ft.Colors.ON_SURFACE_VARIANT)
            ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, alignment=ft.MainAxisAlignment.CENTER, spacing=8, expand=True),
            expand=True,
        )
...
def create_search_bar(page, on_search):
            ...
            [
                ...
                ft.Container(
                    content=ft.Column([
                        ...
                        ft.FilledTonalButton("Back to Events", on_click=lambda e: page.go("/"), icon=ft.Icons.ARROW_BACK)
                ...
                ft.Card(
                    ...
                    content=ft.Container(
                        ...
                        content=ft.Column([
                            ...
                            ft.FilledTonalButton("Book Now",icon=ft.Icons.BOOK_OUTLINED, on_click=lambda e, sid=show['EventID']: page.go(f"/seating/{sid}"), expand=True, height=36)
...
def create_seating_view(page, event_id):
    ...
    return ft.View(
        ...
        [
            ...
            ft.Row([ft.FilledTonalButton("Book Selected Seats", on_click=book_clicked, icon=ft.Icons.CHECK_CIRCLE_OUTLINE)], 
                  alignment=ft.MainAxisAlignment.CENTER),
...
def create_payment_view(page, event_id, encoded_labels):
    ...
    return ft.View(
        ...
        [
            ...
            ft.Row([ft.FilledTonalButton("Pay", on_click=lambda e: pay_clicked(event_id, selected_labels), icon=ft.Icons.CREDIT_CARD)],
                  alignment=ft.MainAxisAlignment.CENTER)
        ],
        horizontal_alignment=ft.CrossAxisAlignment.CENTER
    )
                ...
                content=ft.Column([
                    ...
                    ticket_list if tickets else ft.Container(
                        content=ft.Column([
                            ...
                            ft.FilledTonalButton("Browse Events", on_click=lambda e: page.go("/"), icon=ft.Icons.EVENT_AVAILABLE)
...
def main(page: ft.Page):
    ...
    # page.theme = ft.Theme(color_scheme_seed=ft.Colors.BLUE_50)
